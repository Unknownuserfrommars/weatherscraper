...
def get_weather(city, celsius=False):
    # Added in 1.0.1 (Initial Public Release): The `Celsius` parameter
    ...
    try:
        ...
        if temp_element:
            ...
            if temperature:
                # Remove degree symbol and assume temperature is in Fahrenheit if not specified otherwise
                temp_str = temperature.text.replace('°', '')
                temp_in_f = float(temp_str)
                if celsius:
                    weather_data['temperature'] = round((temp_in_f - 32) * 5 / 9, 1)  # Convert to Celsius
                else:
        ...
        humidity = soup.find('div', class_=lambda c: c and ('humidity' in c.lower() or 'precip' in c.lower()))
        if humidity:
            humidity_value = humidity.find('span', class_=lambda c: c and 'value' in c.lower())
            weather_data['humidity'] = humidity_value.text.strip() if humidity_value else "N/A"
        else:
            weather_data['humidity'] = "N/A"
            logger.warning("Humidity data not found.")

        wind = soup.find('div', class_=lambda c: c and 'wind' in c.lower())
        if wind:
            wind_speed = wind.find('span', class_=lambda c: c and ('speed' in c.lower() or 'wrapper' in c.lower()))
            weather_data['wind'] = wind_speed.text.strip() if wind_speed else "N/A"
        else:
